...
public class Tallyer {
    ...
    public static Map<String, Integer> tallyTopicsFiltered(List<String> ids, List<String> topics) {
        // here i count how many times each id appears
        Map<String, Integer> idTotals = new HashMap<>();
        for (String id : ids) {
            if (idTotals.containsKey(id)) {
                idTotals.put(id, idTotals.get(id) + 1);
            } else {
                idTotals.put(id, 1);
            }
        }

        // here i only count topics from ids with exactly 2 votes
        Map<String, Integer> counts = new HashMap<>();
        for (int i = 0; i < ids.size(); i++) {
            String id = ids.get(i);
            if (idTotals.get(id) == 2) {
